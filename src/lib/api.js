...
export const getProducts = async (params = {}) => {
    const response = await fetch(`${API_BASE}/api/products${toQueryString(params)}`, {
...
export const getRelatedProducts = async (productId, limit = 6) => {
    const response = await fetch(`${API_BASE}/api/products/${productId}/related${toQueryString({ limit })}`, {
        headers: defaultHeaders,
    });
    if (!response.ok) {
